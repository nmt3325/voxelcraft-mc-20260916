...
test.describe('deterministic world persistence', () => {
	...
	test('break, place, save and reload restore the same world state', async ({ page }) => {
		...
		expect(issues.join(' | '), 'the seeded boot must not log console errors or warnings').toBe('')

		// No test API means the app is broken, so fail loudly instead of skipping.
		...
		await page.reload()
		// The reloaded page must expose the test API again; a miss is a failure.
		await waitForTestApi(page)
		await ready()
		...
		expect(issues.join(' | '), 'the whole loop must not log console errors or warnings').toBe('')
	})
})
