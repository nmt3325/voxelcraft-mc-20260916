 ...
 *
 * Pure data on purpose, so the shipped recipe registry can register the merged
 * table without importing the factory that builds it. `RECIPE_DEFS` is left
 * alone, so the v1 count stays frozen; every id here is new and every smelting
 * input is unique across the merged table, because the shared registry rejects
 * duplicate recipe ids and duplicate smelting inputs.
 */
function shaped(
	id: string,
	width: 1 | 2 | 3,
...
/** v1 plus v2: exactly what the shipped `RECIPES` registry registers. */
export const ALL_RECIPE_DEFS: readonly Recipe[] = Object.freeze([...RECIPE_DEFS, ...RECIPE_V2_DEFS])
export const ALL_RECIPE_DEF_COUNT = ALL_RECIPE_DEFS.length
