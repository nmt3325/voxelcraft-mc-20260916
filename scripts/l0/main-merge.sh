...
if [ -z "${PR_NUM:-}" ]; then
  gh pr create --base main --head integration/mc-20260916 --title "VoxelCraft v1 integration (mc-20260916)" --body-file "$ROOT/pr-body.md" >"$ROOT/logs/pr-create.log" 2>&1
  echo "RC pr_create=$?"
...
gh pr merge "$PR_NUM" --merge --subject "Merge VoxelCraft v1 integration (mc-20260916)" --body "Full gate green on $INTEG" >"$ROOT/logs/pr-merge.log" 2>&1
RC=$?
echo "RC merge=$RC"
